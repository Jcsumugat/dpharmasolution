...
class ChatConversationController extends Controller
{
    ...
    public function sendMessage(Request $request, $conversationId): JsonResponse
    {
        ...
        try {
            ...
            // Custom validation logic - check if we have either message or attachments
            $hasMessage = !empty(trim($request->message));
            $hasFiles = $request->hasFile('attachments');

            if (!$hasMessage && !$hasFiles) {
                \Log::error('Neither message nor attachments provided', [
            ...
            // Validate request data with more flexible rules
            ...
            $messageData = [
                'conversation_id' => $conversationId,
                'admin_id' => Auth::id(),
                'message' => $request->message ?: '', // Use empty string if null/empty
