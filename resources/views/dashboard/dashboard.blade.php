...
<body>
    ...
    <main>
        <div class="page-title" style="display: flex;">
            ...
            <div class="order-type-filters">
                ...
                <button class="filter-btn" data-filter="online">Online Sales</button>
                <button class="filter-btn" data-filter="pos">Walk In Sales</button>
            </div>
        </div>
        ...
        <section class="right-panel">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                ...
                <div id="connectionStatus" style="font-size: 12px; display: flex; align-items: center;">
                    <span class="status-indicator" id="statusDot"
                        style="display: inline-block; width: 8px; height: 8px; background: #22c55e; border-radius: 50%; margin-right: 5px;"></span>
            ...
            <div class="activities" id="activitiesContainer">
                ...
                @if ($activities['pos_transactions_today'] > 0)
                    <div class="activity-box success" id="posTransactions">
                        🛒 <strong>{{ $activities['pos_transactions_today'] }}</strong>
                        walk-in transaction{{ $activities['pos_transactions_today'] != 1 ? 's' : '' }} today
                    </div>
                @endif
    ...
    <script type="text/javascript">
        ...
        window.currentFilter = urlParams.get('filter') || 'all';
    </script>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Load dashboard with initial filter if not 'all'
            if (window.currentFilter !== 'all') {
                refreshDashboardWithFilter(window.currentFilter);
            } else {
                startAutoRefresh();
            }
        });
        ...
        function setupFilters() {
            ...
            // Set the active button based on current filter
            filterButtons.forEach(btn => {
                btn.classList.remove('active');
                if (btn.dataset.filter === window.currentFilter) {
            ...
            filterButtons.forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    if (window.currentFilter === 'all') {
                        ...
                        url.searchParams.set('filter', window.currentFilter);
                    }
        ...
        function refreshDashboardWithFilter(filter) {
            ...
            fetch(`{{ route('admin.dashboard.stats') }}?filter=${filter}`, {
                    method: 'GET',
                    headers: {
                ...
                .catch(error => {
                    ...
                    if (connectionRetries < maxRetries) {
                        ...
                        setTimeout(() => refreshDashboardWithFilter(filter), 5000);
                    } else {
        ...
        function startAutoRefresh() {
            refreshInterval = setInterval(() => refreshDashboardWithFilter(window.currentFilter), refreshIntervalTime);
        }
        ...
        function updateStatsCards(stats) {
            document.getElementById('totalSales').textContent = '₱' + Number(stats.total_sales).toLocaleString(undefined, {
                minimumFractionDigits: 2
            });
            document.getElementById('totalProducts').textContent = Number(stats.total_products).toLocaleString();
            document.getElementById('totalProfit').textContent = '₱' + Number(stats.total_profit).toLocaleString(
            undefined, {
                minimumFractionDigits: 2
            });
        }
