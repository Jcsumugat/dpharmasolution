    ...
    <div class="table-wrapper">
      @if($suppliers->isEmpty())
        <div class="no-suppliers">
          ...
          <div style="font-size: 0.875rem; opacity: 0.7; margin-top: 0.5rem;">Add your first supplier to get started</div>
        </div>
      @else
          ...
          <tbody>
            @foreach($suppliers as $supplier)
              <tr>
                ...
                <td>
                  <div class="dropdown">
                    <button class="dropdown-toggle" onclick="toggleDropdown(event)" aria-label="Actions">&#8943;</button>
                    <div class="dropdown-menu">
                      <button class="dropdown-item" onclick='editSupplier(@json($supplier))'>
                        <span>✏️</span> Edit
                      </button>
                      <form action="{{ route('suppliers.destroy', $supplier->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this supplier?')" style="margin: 0;">
                        @csrf
                        @method('DELETE')
